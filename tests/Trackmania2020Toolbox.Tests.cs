...
public class ToolboxTests
{
    ...
    public async Task HandleExportCampaignMedalsAsync_ShouldEscapeQuotesInCsv()
    {
        ...
        var campaignItemMock = new Mock<ICampaignItem>();
        campaignItemMock.Setup(c => c.Id).Returns(1);
        campaignItemMock.Setup(c => c.Name).Returns("Campaign \"with quotes\"");
        ...
        var mapMock = new Mock<IMap>();
        mapMock.Setup(m => m.Name).Returns("Track \"name\"");
        mapMock.Setup(m => m.MapUid).Returns("uid1");

        var campaignMock = new Mock<ICampaign>();
        campaignMock.Setup(c => c.Playlist).Returns(new[] { mapMock.Object });
        campaignMock.Setup(c => c.Name).Returns("Campaign \"with quotes\"");
        ...
        _fsMock.Verify(f => f.WriteAllLinesAsync(It.IsAny<string>(), It.Is<IEnumerable<string>>(lines =>
            lines.Any(l => l.Contains("Campaign \"\"with quotes\"\"") && l.Contains("Track \"\"name\"\""))
        )), Times.Once);
    }

    [Fact]
    public async Task GetCachedAsync_ShouldUseHashedKeyForFilename()
    {
        ...
        // Key is "weekly_shorts_page_1"
        // SHA256 of "weekly_shorts_page_1" is 79354087...
        fs.Verify(f => f.FileExists(It.Is<string>(s => !s.Contains("weekly_shorts_page_1") && s.EndsWith(".json"))), Times.Once);
    }

    [Fact]
    public async Task RealConfigService_ShouldBeThreadSafe()
    {
        var fs = new Mock<IFileSystem>();
        var service = new RealConfigService(fs.Object);
        var scriptDir = "/test";
        var config = Config.Default;

        fs.Setup(f => f.ReadAllTextAsync(It.IsAny<string>())).ReturnsAsync("App.ForceOverwrite = true");
        fs.Setup(f => f.FileExists(It.IsAny<string>())).Returns(true);

        var t1 = service.LoadConfigAsync(scriptDir);
        var t2 = service.SaveConfigAsync(scriptDir, config);
